...
module.exports.run = function run(options) {
  ...
  // TODO: Can we just have a camelizeKeys method or something?
  //
  // TODO: This doesn't work, it's options.options['live-reload-port']
  options.liveReloadPort = options.liveReloadPort || (parseInt(options.port, 10) - 8000) + LIVE_RELOAD_PORT;
  options.proxyPort = options.options['proxy-port'];
  options.proxyHost = options.options['proxy-host'] || DEFAULT_HOST;
